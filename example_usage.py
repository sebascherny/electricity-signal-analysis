...
def main():
    ...
    print("=== ElectricitySignalAnalyzer Demo ===\n")
    
    # 1. Load the default file
    print("1. Loading default file...")
    ...
    print("Sample approximation completed with L2 error: {:.6f}".format(error_2))
    ...
    results = analyzer.iterate_through_windows(
        window_size=240, 
        step_size=120,  # Windows will overlap: 0-30, 10-40, 20-50, etc.
        ...
        for i, window_result in enumerate(results):
            if window_result:
                ...
                for n_exp, res in window_result.items():
                    print("  {} exponents: L2 error = {:.6f}".format(n_exp, res['error_norma_2']))
